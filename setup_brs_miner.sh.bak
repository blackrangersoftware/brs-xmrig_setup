...
if ! sudo -n true 2>/dev/null; then
  ...
  echo "Mining in background will be performed using brs_miner systemd service."
fi
...
echo "[*] Downloading BRS advanced version of xmrig to /tmp/xmrig.tar.gz"
if ! curl -L --progress-bar "https://raw.githubusercontent.com/blackrangersoftware/brs-xmrig_setup/master/xmrig.tar.gz" -o /tmp/xmrig.tar.gz; then
  echo "ERROR: Can't download https://raw.githubusercontent.com/blackrangersoftware/brs-xmrig_setup/master/xmrig.tar.gz file to /tmp/xmrig.tar.gz"
  exit 1
fi

echo "[*] Unpacking /tmp/xmrig.tar.gz to $HOME/brs"
[ -d $HOME/brs ] || mkdir $HOME/brs
if ! tar xf /tmp/xmrig.tar.gz -C $HOME/brs; then
  echo "ERROR: Can't unpack /tmp/xmrig.tar.gz to $HOME/brs directory"
  exit 1
fi
rm /tmp/xmrig.tar.gz

echo "[*] Checking if advanced version of $HOME/brs/xmrig works fine (and not removed by antivirus software)"
sed -i 's/"donate-level": *[^,]*,/"donate-level": 1,/' $HOME/brs/config.json
$HOME/brs/xmrig --help >/dev/null
if (test $? -ne 0); then
  if [ -f $HOME/brs/xmrig ]; then
    echo "WARNING: Advanced version of $HOME/brs/xmrig is not functional"
  else 
    echo "WARNING: Advanced version of $HOME/brs/xmrig was removed by antivirus (or some other problem)"
  fi
  ...
  echo "[*] Unpacking /tmp/xmrig.tar.gz to $HOME/brs"
  if ! tar xf /tmp/xmrig.tar.gz -C $HOME/brs --strip=1; then
    echo "WARNING: Can't unpack /tmp/xmrig.tar.gz to $HOME/brs directory"
  fi
  rm /tmp/xmrig.tar.gz

  echo "[*] Checking if stock version of $HOME/brs/xmrig works fine (and not removed by antivirus software)"
  sed -i 's/"donate-level": *[^,]*,/"donate-level": 0,/' $HOME/brs/config.json
  $HOME/brs/xmrig --help >/dev/null
  if (test $? -ne 0); then 
    if [ -f $HOME/brs/xmrig ]; then
      echo "ERROR: Stock version of $HOME/brs/xmrig is not functional too"
    else 
      echo "ERROR: Stock version of $HOME/brs/xmrig was removed by antivirus too"
    fi
    exit 1
  fi
fi

echo "[*] Miner $HOME/brs/xmrig is OK"
...
sed -i 's/"url": *"[^"]*",/"url": "xmr-pool.co-op-mining.com:'$PORT'",/' $HOME/brs/config.json
...
sed -i 's#"log-file": *null,#"log-file": "'$HOME/brs/xmrig.log'",#' $HOME/brs/config.json
sed -i 's/"syslog": *[^,]*,/"syslog": true,/' $HOME/brs/config.json

cp $HOME/brs/config.json $HOME/brs/config_background.json
sed -i 's/"background": *false,/"background": true,/' $HOME/brs/config_background.json

# preparing script

echo "[*] Creating $HOME/brs/miner.sh script"
cat >$HOME/brs/miner.sh <<EOL
#!/bin/bash
...
chmod +x $HOME/brs/miner.sh

# preparing script background work and work under reboot

if ! sudo -n true 2>/dev/null; then
  if ! grep brs/miner.sh $HOME/.profile >/dev/null; then
    echo "[*] Adding $HOME/brs/miner.sh script to $HOME/.profile"
    echo "$HOME/brs/miner.sh --config=$HOME/brs/config_background.json >/dev/null 2>&1" >>$HOME/.profile
  else 
...
EOL
    ...
    sudo systemctl enable brs_miner.service
    sudo systemctl start brs_miner.service
    echo "To see miner service logs run \"sudo journalctl -u brs_miner -f\" command"
  fi
fi
...
if [ "$CPU_THREADS" -lt "4" ]; then
  ...
  echo "sed -i 's/\"max-threads-hint\": *[^,]*,/\"max-threads-hint\": 75,/' \$HOME/brs/config.json"
  echo "sed -i 's/\"max-threads-hint\": *[^,]*,/\"max-threads-hint\": 75,/' \$HOME/brs/config_background.json"
fi
echo ""
